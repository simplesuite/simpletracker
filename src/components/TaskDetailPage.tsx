...
export default function TaskDetailPage() {
    ...
    const [recurrenceAnchor, setRecurrenceAnchor] = useState<'due_date' | 'completed_date'>('due_date');
    ...
    const checkIfShared = async (t: Task) => {
        try {
            ...
            const shared = isSharedItem(t, currentUser.recordID, noteShares, projectShares);
            setIsShared(shared);

            // If shared and offline, show message
            if (shared && !isOnline) {
                setOfflineMessage('Shared items require an internet connection to view and edit.');
            } else {
    ...
    const saveBody = useCallback(async (newBody: string) => {
        if (!id) return;
    ...
    const handleRecurrenceUnitChange = async (value: 'days' | 'weeks' | 'months') => {
        if (!id) return;
    ...
    const handleRecurrenceAnchorChange = async (value: 'due_date' | 'completed_date') => {
        if (!id) return;
    ...
    const handleAddSubtask = async () => {
        ...
        if (!validation.valid) {
            setSubtaskError(validation.error || 'Invalid subtask title');
            return;
        }
        setSubtaskError(null);
        const result = await addSubtask(id, newSubtaskTitle);
        if (result) {
            setNewSubtaskTitle('');
        } else {
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
            <Box sx={{ pb: 10, maxWidth: 600, mx: 'auto' }}>
                ...
                <Typography variant="h6" gutterBottom>
                    Subtasks ({taskSubtasks.length}/50)
                </Typography>
