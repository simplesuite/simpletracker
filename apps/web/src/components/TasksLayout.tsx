 ...
 *
 * Split-view only kicks in on large screens (md+) AND when a task is selected.
 * In that case the list and the selected task's detail sit side-by-side, each
 ...
 *
 * When no task is selected — or on small screens — the list renders exactly as
 * before (full width, centered). On small screens a selected task opens the
...
export default function TasksLayout() {
    ...
    if (!isLargeScreen) {
        return id ? <TaskDetailPage /> : <TasksPage />;
    }
    ...
    return (
        <Box sx={{ display: 'flex', gap: 3, alignItems: 'flex-start', justifyContent: 'center', width: '100%' }}>
            ...
            <Box sx={{ flex: 1, minWidth: 0, maxWidth: COLUMN_MAX_WIDTH }}>
                <TaskDetailPage key={id} id={id} onBack={handleCloseDetail} />
            </Box>
        </Box>
    );
}
